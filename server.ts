...
import { consultarCnpj } from './server/cnpjService.js';
...
async function startServer() {
    ...
    try {
      ...
      res.json({
        ...
        sync_status: {
          ...
          mensagem: 'Banco SQLite sincronizado em nuvem corporativa'
        }
      });
    } catch (err: any) {
  ...
  app.get('/api/companies/:cnpj', (req, res) => {
    try {
      ...
      if (!company) {
        return res.status(404).json({ error: 'Empresa não encontrada no banco SQLite local' });
      }
      res.json(company);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
  ...
  app.post('/api/companies', (req, res) => {
    try {
      ...
      res.json({ message: 'Empresa salva no SQLite com sucesso', company });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });
  ...
  app.delete('/api/companies/:cnpj', (req, res) => {
    try {
      ...
      res.json({ message: 'Empresa removida do SQLite com sucesso' });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });

  // Clear all companies (e.g. wipe sample data to start clean)
  app.post('/api/companies/clear-all', (req, res) => {
    try {
      clearAllCompanies(db);
      res.json({ message: 'Todas as empresas foram removidas do SQLite local com sucesso.' });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  });
  ...
  app.post('/api/sync/trigger', (req, res) => {
    try {
      const companies = getAllCompanies(db);
      // Update pending items sync status
      companies.forEach(c => {
      ...
      logSync(db, {
        ...
        mensagem: 'Sincronização manual com o Cofre em Nuvem/Google Drive concluída com sucesso'
      });

      res.json({
        sucesso: true,
        mensagem: 'Banco de dados SQLite sincronizado em nuvem corporativa.',
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 Servidor Corporativo rodando na porta ${PORT} [http://localhost:${PORT}]`);
  });
}
